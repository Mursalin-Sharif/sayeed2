...
export function AdminLandingMedia() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      setNotice(editingId ? 'Photo/video updated.' : 'Photo/video saved to the landing page.')
      resetForm()
    } catch (error) {
  ...
  return (
    <section id="media" className="scroll-mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
      <p className="text-sm font-semibold text-zinc-200">Landing photos & videos</p>
      <p className="mt-1 text-xs text-zinc-500">
        Upload here only. These show on /offer — not on the home product cards.
      </p>
      {notice ? (
        <p className={`mt-2 text-sm font-semibold ${notice.includes('fail') ? 'text-red-400' : 'text-emerald-400'}`}>
          {notice}
        </p>
      ) : null}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_280px]">
          ...
          {media
            ...
            .map((item) => (
              <article key={item.id} className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
                ...
                <div className="p-3">
                  <p className="font-semibold text-zinc-100">{item.title || 'Untitled'}</p>
        ...
        <form onSubmit={onSubmit} className="space-y-3 rounded-2xl border border-white/10 bg-black/20 p-4">
          <h2 className="font-semibold text-zinc-100">{editingId ? 'Edit file' : 'Upload photo or video'}</h2>
          <select
            value={form.type}
            onChange={(e) => setForm({ ...form, type: e.target.value === 'video' ? 'video' : 'image' })}
            className="w-full rounded-xl bg-black/30 px-3 py-2 text-zinc-100"
          >
          ...
          <div className="flex gap-2">
            ...
            >
              {saving ? 'Saving...' : editingId ? 'Update file' : 'Save file'}
            </button>
            {editingId ? (
              <button type="button" onClick={resetForm} className="rounded-xl bg-white/10 px-3 text-zinc-100">
                Cancel
              </button>
            ) : null}
          </div>
